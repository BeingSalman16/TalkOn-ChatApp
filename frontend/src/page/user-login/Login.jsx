...
import { sendOtp, updateUserProfile, verifyOtp } from '../../services/auth.service'
...
const {
  ...
  const ProgressBar = () => (
    <div className={`w-full ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-200'} rounded-full h-2.5 mb-6`}>
      <div
        className="bg-gradient-to-r from-[#00E5FF] to-[#25D366] h-2.5 rounded-full transition-all duration-500 ease-in-out"
            ...
            <div className="relative">
              <div className="flex">
                <div className="relative w-1/3">
                  <button
                    type="button"
                    className={`flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center ${theme === 'dark' ? 'text-white bg-gray-700 border-gray-600 focus:ring-gray-500' : 'text-gray-900 bg-gray-100 border-gray-300 focus:ring-gray-300'} border rounded-s-lg focus:ring-4 focus:outline-none`}
                    ...
                    <div  className={`absolute z-10 w-full mt-1 ${theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300'} border rounded-md shadow-lg max-h-60 overflow-auto`}>
                      <div className={`sticky top-0 ${theme === 'dark' ? 'bg-gray-700' : 'bg-white'} p-2`}>
                        <input
                          ...
                          className={`w-full px-2 py-1 border ${theme === 'dark' ? 'bg-gray-600 border-gray-500 text-white' : 'bg-white border-gray-300'} rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-300`}
                        />
                      </div>
                      {filteredCountries.map((country) => (
                        <button
                          key={country.alpha2}
                          type="button"
                          className={`w-full text-left px-3 py-2 ${theme === 'dark' ? 'hover:bg-gray-600' : 'hover:bg-gray-100'} focus:outline-none`}
                          onClick={() => {
                ...
                <input
                  ...
                  className={`w-2/3 px-4 py-2 border ${theme === 'dark' ? 'bg-gray-700 border-gray-600 text-white focus:ring-gray-500' : 'bg-white border-gray-300 focus:ring-gray-300'} rounded-md focus:outline-none focus:ring-2 ${
...
<input
  ...
  onChange={(e) => setEmail(e.target.value)}
   className={`w-full bg-transparent focus:outline-none ${theme === 'dark' ? 'text-white' : 'text-black'}focus:ring-gray-500`}
            ...
            <div className="flex justify-between">
              {otp.map((digit, index) => (
                <input
                  ...
                  className={`w-12 h-12 text-center border ${theme === 'dark' ? 'bg-gray-700 border-gray-600 text-white focus:ring-gray-500' : 'bg-white border-gray-300 focus:ring-gray-200'} rounded-md focus:outline-none focus:ring-2 ${
